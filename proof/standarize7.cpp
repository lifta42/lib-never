...
template <typename T> struct _Last<T> { using Type = T; };
...
template <typename T>
struct ContTrait : public ContTrait<decltype(&T::operator())> {};
template <typename T, typename C> struct ContTrait<Never (C::*)(T &&) const> {
  using Type = T;
};

template <typename T>
struct ProcTrait : public ProcTrait<decltype(&T::operator())> {};
template <typename T, typename U, typename C>
struct ProcTrait<Never (C::*)(T &&, Cont<U>) const> {
  using ArgType = T;
  using RetType = U;
};

template <typename T>
struct GenProcTrait : public GenProcTrait<decltype(&T::operator())> {};
template <typename T, typename U, typename C>
struct GenProcTrait<Never (C::*)(T &&, Cont<U>) const> {
  template <template <typename...> class V> using ArgType = V<T>;
  using RetType = U;
  using NestedType = Proc<T, U>;
};
template <typename T, typename... Us, typename C>
struct GenProcTrait<Never (C::*)(T &&, Us...) const> {
  template <typename... Ts> struct Bar;
  template <typename... Ts> struct Bar<Ts &&...> {
    template <template <typename...> class V>
    using Type = V<Ts..., typename ContTrait<Last<Us...>>::Type>;
  };
  using TailProc =
      typename Init<Us...>::template Type<Bar>::template Type<GenProc>;
  template <typename... Ts> struct Foo {
    template <template <typename...> class V> using Type = V<T, Ts...>;
  };
  template <template <typename...> class V>
  using ArgType =
      typename GenProcTrait<TailProc>::template ArgType<Foo>::template Type<V>;
...
using Nested = typename GenProcTrait<GenProc<Ts...>>::NestedType;
...
const bool StandFoo<T, Us...>::flag = false;
template <typename GP>
using SingleArg = typename GenProcTrait<GP>::template ArgType<StandFoo>;

template <typename GP>
auto stand_impl(GP &&gp, Cont<NestedProc<GP>> pass)
...
auto stand_impl(GP &&gp, Cont<NestedProc<GP>> pass)
  ...
  using W = typename GenProcTrait<GP>::template ArgType<
      StandFoo>::template BarType<R>;
...
int main() {
  stand([](int &&x, int &&y, Cont<int> pass) { pass(x + y); },
        [](auto &&proc) {
          proc(3, [](auto &&proc) {
            proc(5, [](int &&eight) { cout << eight << endl; });
          });
        });
}
